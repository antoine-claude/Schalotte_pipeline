...
IGNORE_PATTERNS = ('*.pyc', 'tmp*', '*__pycache__*', '.git', '.vscode', '.gitignore', '.gitattributes', 'build.py', 'schalotte_pipeline_updater')


# Add root folder to path
if destination_path[-1] in ['/', '\\']:
    destination_path += "Schalotte_pipeline-" + version_number
else:
    destination_path += "/Schalotte_pipeline-" + version_number
...
addon_folder_name = "Schalotte_pipeline"
destination_addon_path = os.path.join(destination_path, addon_folder_name)

dir_path = os.path.dirname(os.path.realpath(__file__))

# Create the root destination folder
os.makedirs(destination_path, exist_ok=True)

# Copy all files except ignored patterns into the addon subfolder
shutil.copytree(dir_path, destination_addon_path, ignore=shutil.ignore_patterns(*IGNORE_PATTERNS))

if pack_to_zip:
    shutil.make_archive(zip_path, "zip", root_dir=destination_path)
